import { HttpStatus } from '@nestjs/common';
import { verifyAppleIdTokenAndBuildOAuthProfile } from 'src/contexts/iam/application/auth/helpers/apple-oauth.helper';
import { FoodaException } from 'src/contexts/shared/domain/exceptions/identity.exception';
import * as jwt from 'jsonwebtoken';

describe('verifyAppleIdTokenAndBuildOAuthProfile', () => {
  const mockAudience = 'com.test.app';

  function createAppleIdToken(payload: object): string {
    return jwt.sign(payload, 'test-secret', { algorithm: 'HS256' });
  }

  describe('error cases', () => {
    it('should throw Ex1101 when idToken is empty', async () => {
      await expect(
        verifyAppleIdTokenAndBuildOAuthProfile({
          idToken: '',
          audience: mockAudience,
        }),
      ).rejects.toThrow(FoodaException);
      await expect(
        verifyAppleIdTokenAndBuildOAuthProfile({
          idToken: '',
          audience: mockAudience,
        }),
      ).rejects.toMatchObject({
        status: HttpStatus.BAD_REQUEST,
        response: expect.objectContaining({ code: 'ID-1101' }),
      });
    });

    it('should throw Ex1101 when idToken is null', async () => {
      await expect(
        verifyAppleIdTokenAndBuildOAuthProfile({
          idToken: null as any,
          audience: mockAudience,
        }),
      ).rejects.toThrow(FoodaException);
    });

    it('should throw Ex1101 when idToken is undefined', async () => {
      await expect(
        verifyAppleIdTokenAndBuildOAuthProfile({
          idToken: undefined as any,
          audience: mockAudience,
        }),
      ).rejects.toThrow(FoodaException);
    });

    it('should throw Ex1100 when idToken is not a string', async () => {
      await expect(
        verifyAppleIdTokenAndBuildOAuthProfile({
          idToken: 123 as any,
          audience: mockAudience,
        }),
      ).rejects.toMatchObject({
        status: HttpStatus.BAD_REQUEST,
        response: expect.objectContaining({ code: 'ID-1100' }),
      });
    });

    it('should throw Ex1099 when idToken is invalid format', async () => {
      await expect(
        verifyAppleIdTokenAndBuildOAuthProfile({
          idToken: 'invalid-token',
          audience: mockAudience,
        }),
      ).rejects.toMatchObject({
        status: HttpStatus.UNAUTHORIZED,
        response: expect.objectContaining({ code: 'ID-1099' }),
      });
    });

    it('should throw Ex1099 when audience does not match', async () => {
      const now = Math.floor(Date.now() / 1000);
      const token = createAppleIdToken({
        iss: 'https://appleid.apple.com',
        aud: 'wrong-audience',
        exp: now + 3600,
        iat: now,
        sub: '12345',
        email: 'test@example.com',
        email_verified: 'true',
      });

      await expect(
        verifyAppleIdTokenAndBuildOAuthProfile({
          idToken: token,
          audience: mockAudience,
        }),
      ).rejects.toMatchObject({
        status: HttpStatus.UNAUTHORIZED,
        response: expect.objectContaining({ code: 'ID-1099' }),
      });
    });

    it('should throw Ex1099 when token is expired', async () => {
      const expiredTime = Math.floor(Date.now() / 1000) - 3600;
      const token = createAppleIdToken({
        iss: 'https://appleid.apple.com',
        aud: mockAudience,
        exp: expiredTime,
        iat: expiredTime - 7200,
        sub: '12345',
        email: 'test@example.com',
        email_verified: 'true',
      });

      await expect(
        verifyAppleIdTokenAndBuildOAuthProfile({
          idToken: token,
          audience: mockAudience,
        }),
      ).rejects.toMatchObject({
        status: HttpStatus.UNAUTHORIZED,
        response: expect.objectContaining({ code: 'ID-1099' }),
      });
    });

    it('should throw Ex1099 when sub is missing', async () => {
      const now = Math.floor(Date.now() / 1000);
      const token = createAppleIdToken({
        iss: 'https://appleid.apple.com',
        aud: mockAudience,
        exp: now + 3600,
        iat: now,
        sub: '',
        email: 'test@example.com',
        email_verified: 'true',
      });

      await expect(
        verifyAppleIdTokenAndBuildOAuthProfile({
          idToken: token,
          audience: mockAudience,
        }),
      ).rejects.toMatchObject({
        status: HttpStatus.UNAUTHORIZED,
        response: expect.objectContaining({ code: 'ID-1099' }),
      });
    });

    it('should throw Ex1099 when email is missing', async () => {
      const now = Math.floor(Date.now() / 1000);
      const token = createAppleIdToken({
        iss: 'https://appleid.apple.com',
        aud: mockAudience,
        exp: now + 3600,
        iat: now,
        sub: '12345',
        email: '',
        email_verified: 'true',
      });

      await expect(
        verifyAppleIdTokenAndBuildOAuthProfile({
          idToken: token,
          audience: mockAudience,
        }),
      ).rejects.toMatchObject({
        status: HttpStatus.UNAUTHORIZED,
        response: expect.objectContaining({ code: 'ID-1099' }),
      });
    });
  });

  describe('success cases', () => {
    it('should return OAuthProfile with apple provider', async () => {
      const now = Math.floor(Date.now() / 1000);
      const token = createAppleIdToken({
        iss: 'https://appleid.apple.com',
        aud: mockAudience,
        exp: now + 3600,
        iat: now,
        sub: '12345',
        email: 'test@example.com',
        email_verified: 'true',
      });

      const result = await verifyAppleIdTokenAndBuildOAuthProfile({
        idToken: token,
        audience: mockAudience,
      });

      expect(result).toEqual({
        provider: 'apple',
        providerAccountId: '12345',
        email: 'test@example.com',
        firstName: 'Apple',
        lastName: 'User',
        avatarUrl: undefined,
        metadata: {
          providerRawProfile: expect.objectContaining({
            iss: 'https://appleid.apple.com',
            aud: mockAudience,
            sub: '12345',
            email: 'test@example.com',
          }),
          emailVerified: true,
        },
      });
    });

    it('should set emailVerified to false when email_verified is not true', async () => {
      const now = Math.floor(Date.now() / 1000);
      const token = createAppleIdToken({
        iss: 'https://appleid.apple.com',
        aud: mockAudience,
        exp: now + 3600,
        iat: now,
        sub: '12345',
        email: 'test@example.com',
        email_verified: 'false',
      });

      const result = await verifyAppleIdTokenAndBuildOAuthProfile({
        idToken: token,
        audience: mockAudience,
      });

      expect(result.metadata.emailVerified).toBe(false);
    });

    it('should handle token without email_verified field', async () => {
      const now = Math.floor(Date.now() / 1000);
      const token = createAppleIdToken({
        iss: 'https://appleid.apple.com',
        aud: mockAudience,
        exp: now + 3600,
        iat: now,
        sub: '12345',
        email: 'test@example.com',
      });

      const result = await verifyAppleIdTokenAndBuildOAuthProfile({
        idToken: token,
        audience: mockAudience,
      });

      expect(result.provider).toBe('apple');
      expect(result.email).toBe('test@example.com');
    });
  });
});
